...
function wrap(u:string,p:string,r:string){return `/api/click?partner=${encodeURIComponent(p)}&to=${encodeURIComponent(u)}&ref=${encodeURIComponent(r)}`;}
export default function Page(){
  ...
  const faq = {"@context":"https://schema.org","@type":"FAQPage","mainEntity":[
    {"@type":"Question","name":"¿Medina o zona moderna?","acceptedAnswer":{"@type":"Answer","text":"Medina para riads y ambiente tradicional; Gueliz/Hivernage para hoteles modernos y vida nocturna."}}
  ]};
  return (
    <main className="max-w-3xl mx-auto px-4 py-10 space-y-8">
      <script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(faq)}}/>
      <h1 className="text-3xl font-semibold">Hoteles en {city}</h1>
      <p className="text-slate-700">Medina, Gueliz, Hivernage… elige según tu plan.</p>
      <div className="flex gap-3">
        <Link href={wrap(booking,"booking",ref)} target="_blank" className="btn btn-brand">Booking</Link>
        <Link href={wrap(agoda,"agoda",ref)} target="_blank" className="btn btn-ghost">Agoda</Link>
      </div>
    </main>
  );
}
